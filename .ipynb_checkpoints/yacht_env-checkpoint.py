...
class YachtEnv(Env):
    ...
    def step(self, action):
        ...
        if self.yacht_data is not None:
            self.yacht_data.new_transition(state_prime, obs, self.last_action, rew)
        info = dict()
    ...
    def convert_state(self, state):
        ...
        :param state: Global state of the yacht
        """
        yacht_point = Point((state[0], state[1]))
        side = np.sign(state[1] - self.point_a[1])
        d = yacht_point.distance(self.guideline)  # meters
    ...
    def end(self, state_prime, obs):
        if not self.observation_space.contains(obs) or -1 > state_prime[0] or state_prime[0] > self.max_x_episode[0] or 160 < state_prime[1] or state_prime[1]< -160:
            ...
            if self.yacht_data is not None:
                if self.yacht_data.iterations > 0:
                    self.yacht_data.save_experiment(self.name_experiment)
            return True
        else:
            return False
    ...
    def reset(self):
        ...
        if self.yacht_data is not None:
            if self.yacht_data.iterations > 0:
                self.yacht_data.save_experiment(self.name_experiment)
            self.yacht_data.new_iter(state, self.convert_state(state), np.zeros(len(self.last_action)), np.array([0]))
        if self.viewer is not None:
            self.viewer.end_episode()
    ...
    def set_save_experice(self, name='experiment_ssn_ddpg_10iter'):
        ...
        self.yacht_data = YachtExperiment()
